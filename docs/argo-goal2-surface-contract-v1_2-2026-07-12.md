# Argo Goal 2 Surface Contract v1.2
**July 12, 2026. Supersedes v1.1 (July 12, 2026), which stays archived at
`docs/argo-goal2-surface-contract-v1_1-2026-07-12.md`. The route map and
...
**What v1.2 adds:** deck-owned stars (star controls on `qdeck-card`,
seal parity at 25 or more, no fleece edge on decks) and the
any-org-member deck assignment path via `set_stack_deck`. **What v1.1
added:** Canvas as a legitimate fourth library surface, QDeck as a
first-class entity, the stage field, and three new behavioral guarantees
(canvas spatial memory, loose-stack positioning, deck
...
### Social
38. `star-button` (on qstack cards/pages AND on `qdeck-card` and the
    deck page, v1.2; reflects count), `star-count`, `follow-button`,
    `post-input`, `post-submit`, `post-item`,
    `profile-visibility-toggle`, `go-public-prompt` (shown on first
    public QStack).
...
6. Stars: one per user per QStack AND one per user per QDeck (v1.2),
   each enforced by a database unique index, counts correct under
   concurrency. A star names exactly one subject, a stack or a deck.
   The gold seal rule is identical for both: 25 or more stars. Decks
   never render the fleece edge (in rotation is a stack-only state;
   decks have no status field to drive it).
...
11. **(v1.2)** Deck assignment and removal go through the
    `set_stack_deck` RPC and are available to any org member, not just
    the stack's owner or an org admin; the RPC changes deck membership
    and nothing else. Direct writes to other stack fields keep their
    existing owner-or-admin rules.
12. **(v1.2)** Stars never touch canvas position and canvas position
    never feeds a star count, in either direction, ever.
